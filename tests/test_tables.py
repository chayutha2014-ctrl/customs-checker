...
def test_ห้ามยืนยันเมื่อจับได้บรรทัดเดียวจากตารางใหญ่():
    """เคส VORETO ที่พัง — ตาราง 5 แถวแต่จับได้บรรทัดเดียว ต้องไม่บอกว่ายืนยัน"""
    spec = [
        [("Q'TY", 300), ("PRICE", 500), ("AMOUNT", 700)],
        [("27.2", 300), ("0.11", 500), ("3.00", 700)],
        [("999", 300), ("888", 500), ("777.00", 700)],
        [("111", 300), ("222", 500), ("333.00", 700)],
        [("444", 300), ("555", 500), ("666.00", 700)],
        [("TOTAL", 200), ("3.00", 700)],
    ]
    r = analyze_invoice(table(spec))
    assert len(r["lines"]) == 1
    assert r["printed"] is None
    assert "ต้องให้คนตรวจ" in r["status"]
